...
local function parse_uri(uri, errs)
    ...
    local host_capture = protocol_schema .. '(' .. host_schema .. ")[:/]" ..
                             path_schema .. "%.git$"
    local path_capture = protocol_schema .. host_schema .. "[:/](" ..
                             path_schema .. ")%.git$"
    local repo = {
        host = uri:match(host_capture),
        path = uri:match(path_capture)
    }
...
local function is_file_compatible_with_revspec(buf_repo_path, revspec, errs)
    if not is_file_in_rev(buf_repo_path, revspec) then
        table.insert(errs, string.format(": '%s' is not in '%s'", buf_repo_path,
                                         revspec))
        return false
    end
...
local function get_rev_for_revspec_if_contains_file(buf_repo_path, revspec, errs)
    ...
    if not rev then
        table.insert(errs, string.format(
                         ": could not retrieve revision for '%s'", revspec))
        return nil
    end
...
function M.get_closest_remote_compatible_rev(buf_repo_path, remote)
    ...
    if is_rev_in_remote("HEAD", remote) then
        local head_rev = get_rev_for_revspec_if_contains_file(buf_repo_path,
                                                              "HEAD", errs)
        if head_rev then return head_rev end
    else
        table.insert(errs, string.format(": current 'HEAD' not in remote '%s'",
                                         remote))
    end
    ...
    -- try remote HEAD
    local remote_rev = get_rev_for_revspec_if_contains_file(buf_repo_path,
                                                            remote, errs)
...
function M.get_repo(remote)
    ...
    if not remote_uri then
        table.insert(errs, string.format(
                         ": cannot retrieve url from remote '%s'", remote))
        return nil
    end
...
function M.get_git_root()
    local root
    local p =
        job:new({command = "git", args = {"rev-parse", "--show-toplevel"}})
...
function M.get_branch_remote()
    ...
    for _, remote in ipairs(remotes) do
        ...
        if get_rev(upstream_revspec) and
            is_rev_in_remote(upstream_revspec, remote) then return remote end
